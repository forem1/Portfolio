...
namespace FileManager
{
    class FileManager
    {
        ...
        private void CreateFile()
        {
            ...
            string currentPath = this.panels[this.activePanelIndex].Path;
            string newName = this.AksName("Введите имя файла: ");
            string newFullName = Path.Combine(currentPath, newName);

            try
            {
                FileStream fs = File.Create(newFullName);
                this.RefreshPannels();
            }
            catch
            {
        ...
        {
            string[] menu = { "F3 Просмотр", "F4 Поиск", "F5 Копия", "F6 Перемещ", "F7 Создать", "F8 Переимен", "F9 Удаление", "F10 Файл", "F11 Выход" };
